...
def get_version_from_file():
    ...
    if not os.path.exists(version_file):
        logger.error(f"Erreur : Le fichier {version_file} n'existe pas.")
        sys.exit(1)
    ...
    try:
        ...
        logger.error(f"Erreur lors de la lecture du fichier version.txt : {e}")
        sys.exit(1)
...
try:
    ...
    logger.info(f"Contenu des notes lu depuis {RELEASE_NOTES_FILE}")
except FileNotFoundError:
    logger.warning(f"Avertissement : Le fichier {RELEASE_NOTES_FILE} est introuvable. Le corps de la release sera générique.")
    release_notes_content = "Publication automatique de cette version."
except Exception as e:
    logger.error(f"Erreur lors de la lecture de {RELEASE_NOTES_FILE} : {e}. Le corps de la release sera générique.")
...
if not GITHUB_TOKEN:
    ...
    logger.error("Erreur: Le token GitHub n'est pas défini dans la variable d'environnement GITHUB_TOKEN.")
    sys.exit(1)
...
def compile_gdj():
    ...
    """
    logger.info("Compilation de GDJ.exe avec PyInstaller (SANS --windowed pour debug)...")
    try:
        cmd = [
            "pyinstaller", 
            "--onefile", 
        ...
        logger.info(f"Commande PyInstaller pour GDJ: {cmd}")
        subprocess.run(
        ...
        logger.info("Compilation de GDJ.exe terminée.")
    except subprocess.CalledProcessError as e:
        logger.error(f"Erreur lors de la compilation de GDJ.exe : {e}")
...
def compile_update_helper():
    ...
    """
    logger.info("Compilation de update_helper (one-folder, simple) avec PyInstaller...")
    ...
    try:
        ...
        logger.info(f"Commande PyInstaller pour update_helper: {cmd}")
        subprocess.run(
            cmd,
            check=True,
            capture_output=True,
        ...
        logger.info("Compilation de update_helper (one-folder, simple) terminée.")
    except subprocess.CalledProcessError as e:
        logger.error(f"Erreur lors de la compilation de update_helper : {e}")
        logger.error(f"stdout: {e.stdout}"); logger.error(f"stderr: {e.stderr}"); sys.exit(1)
    except FileNotFoundError:
        logger.error("Erreur : PyInstaller introuvable..."); sys.exit(1)
...
def compile_innosetup():
    ...
    """
    logger.info("Compilation de l'installateur avec Inno Setup...")
    try:
        subprocess.run([ISCC_PATH, ISS_SCRIPT], check=True)
        logger.info("Compilation de l'installateur réussie.")
    except subprocess.CalledProcessError as e:
        logger.error(f"Erreur lors de la compilation de l'installateur : {e}")
        sys.exit(1)
...
def create_release():
    ...
    logger.info("Création de la release sur GitHub...")
    response = requests.post(API_URL, headers=headers, json=payload)

    if response.status_code == 201:
        data = response.json()
        logger.info("Release créée avec succès !")
        logger.info(f"URL de la release : {data.get('html_url')}")
        return data
    else:
        logger.error(f"Erreur lors de la création de la release : {response.status_code}")
...
def upload_asset(upload_url, asset_path, asset_label=None):
    ...
    with open(asset_path, "rb") as asset_file:
        logger.info(f"Upload de l'asset {filename}...")
        response = requests.post(upload_url, headers=headers, params=params, data=asset_file)

    if response.status_code == 201:
        logger.info(f"L'asset {filename} a été uploadé avec succès.")
        return response.json()
    else:
        logger.error(f"Erreur lors de l'upload de l'asset : {response.status_code}")
        logger.error(response.text)
        sys.exit(1)
...
def main():
    ...
    if not os.path.exists(gdj_path_dist):
        logger.error("Erreur : GDJ.exe n'a pas été trouvé...")
        sys.exit(1)
    ...
    if not os.path.exists(updater_folder_dist):
        logger.error("Erreur : Le dossier de l'updater n'a pas été trouvé dans 'dist'.")
        sys.exit(1)
    ...
    try:
        logger.info(f"Copie de {gdj_path_dist} vers {gdj_path_installer_src}...")
        shutil.copy(gdj_path_dist, gdj_path_installer_src)
        logger.info("Copie GDJ.exe réussie.")
        logger.info(f"Copie du dossier {updater_folder_dist} vers {updater_dest_folder_installer}...")
        shutil.copytree(updater_folder_dist, updater_dest_folder_installer)
        logger.info("Copie du dossier update_helper réussie.")
    except Exception as e:
        logger.error(f"Erreur lors de la copie des exécutables/dossiers vers le dossier installer : {e}")
        sys.exit(1)

    # Étape 2 : Compiler l'installateur via Inno Setup.
    compile_innosetup()
    if not os.path.exists(INSTALLER_OUTPUT):
        logger.error("Erreur : L'installateur n'a pas été trouvé...")
    ...
    logger.info(f"Création et push du tag Git {TAG_NAME}...")
    try:
        ...
        if "already exists" in error_message:
            ...
            try:
                ...
                logger.info(f"Tag {TAG_NAME} poussé avec succès (existait localement).")
            except subprocess.CalledProcessError as push_error:
                 if "already exists" in push_error.stderr.lower() or "up-to-date" in push_error.stderr.lower():
                     logger.warning(f"Le tag {TAG_NAME} existe déjà sur l'origin.")
                 else:
                     logger.error(f"Erreur lors du push du tag existant {TAG_NAME}: {push_error}")
                     logger.error(f"Stderr: {push_error.stderr}")
                     # Décider si on arrête ou pas. Pour l'instant on continue avec prudence.
                     # sys.exit(1)
        else:
            logger.error(f"Erreur lors de la création/push du tag {TAG_NAME}: {e}")
            logger.error(f"Stderr: {e.stderr}")
            logger.error("Arrêt du script car le tag n'a pas pu être créé/poussé.")
            sys.exit(1)
    # ****************************************
    ...
    if os.path.exists(gdj_path_installer_src):
        try:
            ...
            logger.info(f"Nettoyage : {gdj_path_installer_src} supprimé.")
        except Exception as e:
            logger.warning(f"Avertissement : Échec suppression {gdj_path_installer_src}: {e}")
    if os.path.exists(updater_dest_folder_installer):
        try:
            shutil.rmtree(updater_dest_folder_installer)
            logger.info(f"Nettoyage : Dossier {updater_dest_folder_installer} supprimé.")
        except Exception as e:
            logger.warning(f"Avertissement : Échec suppression dossier {updater_dest_folder_installer}: {e}")

    logger.info("Processus de création de release terminé avec succès.")
