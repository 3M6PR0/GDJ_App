...
class DocumentsTypeSelectionController(QObject): # <- Nom de classe mis à jour
    ...
    def __init__(self, view: DocumentsTypeSelectionPage):
        ...
        super().__init__()
        self.view = view
        logger.info("--- DocumentsTypeSelectionController.__init__ START ---")
        
        # --- MODIFICATION: Récupérer les données depuis les Singletons --- 
        try:
            logger.debug("  Attempting to get ConfigData instance...")
            config = ConfigData.get_instance()
            logger.debug(f"  ConfigData instance obtained: {config}")
            logger.debug("  Attempting to get Preference instance...")
            prefs = Preference.get_instance()
            logger.debug(f"  Preference instance obtained: {prefs}")
            ...
            logger.debug(f"  TypeSelectionController: Options Jacmar récupérées de ConfigData: keys={list(self.jacmar_options.keys())}")

            # 3. Valeurs par défaut depuis Preference
            self.default_profile_values = self._extract_default_values(prefs)
            logger.debug(f"  TypeSelectionController: Valeurs par défaut récupérées de Preference: {self.default_profile_values}")
        ...
        logger.info("--- DocumentsTypeSelectionController.__init__ END ---")
    # -------------------------------------------------------
    ...
    def _extract_jacmar_options(self, config: ConfigData) -> dict:
        ...
        logger.debug("--- DTSC: _extract_jacmar_options START ---")
        jacmar_data = {}
        jacmar_config = config.get_top_level_key("jacmar", default={})
        logger.debug(f"  _extract_jacmar_options: Got jacmar_config keys: {list(jacmar_config.keys())}")
        ...
        logger.debug(f"  _extract_jacmar_options: Retrieved plafond_deplacement raw: {plafonds_raw}")
        if isinstance(plafonds_raw, list) and len(plafonds_raw) > 0 and isinstance(plafonds_raw[0], dict):
            ...
            logger.debug(f"  DTSC: Extracted plafond keys: {jacmar_data['plafond_deplacement']}")
        else:
            logger.warning(f"WARN DTSC: Structure inattendue pour 'plafond_deplacement' dans config: {plafonds_raw}")
            jacmar_data["plafond_deplacement"] = [] # Fallback
        
        logger.debug("--- DTSC: _extract_jacmar_options END ---")
        return jacmar_data
    ...
    def _populate_type_selection_combo(self):
        ...
        logger.info("--- _populate_type_selection_combo START --- ")
        logger.debug(f"  Types à ajouter: {self.document_types}")
        try:
            if hasattr(self.view, 'set_document_types') and callable(getattr(self.view, 'set_document_types')):
                logger.debug(f"  Appel de self.view.set_document_types({self.document_types})")
                self.view.set_document_types(self.document_types)
                logger.info("  Appel à view.set_document_types effectué.")
            else:
                logger.error("  Erreur: La vue TypeSelection n'a pas de méthode set_document_types ou elle n'est pas appelable.")
        except Exception as e:
            logger.error(f"  Erreur lors du peuplement du ComboBox Type de document: {e}", exc_info=True)
        logger.info("--- _populate_type_selection_combo END --- ")
    # --------------------------------------------------------------------------
...
logger.info("DocumentsTypeSelectionController defined (uses Singletons)") 