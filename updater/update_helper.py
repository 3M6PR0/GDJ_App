    ...
    try:
        ...
        with open(output_path, 'wb') as f:
            if total_length is None:
                ...
                print("Téléchargement terminé (taille inconnue).")
            else:
                dl = 0
...
def launch_installer(installer_path):
    ...
    """
    print(f"Lancement de l'installateur (sans attente) : {installer_path}")
    try:
        # Utilisation de Popen pour lancer et ne pas attendre
        subprocess.Popen([installer_path])
        print("Installeur lancé (Popen retourné). L'helper va se terminer.")
        return True # Succès du lancement
    except Exception as e:
        # Erreur si Popen ne peut même pas lancer le processus
        print(f"Erreur imprévue lors du lancement (Popen) de l'installateur : {e}")
        return False
...
def main():
    ...
    print(f"Téléchargement vers : {output_installer}")
    ...
    if download_installer(installer_url, output_installer):
        ...
        if launch_installer(output_installer):
             print("Mise à jour terminée (lancement de l'installeur réussi).")
        else:
             print("Échec du lancement de l'installeur téléchargé.")
             input("Appuyez sur Entrée pour quitter...") # Pause
             sys.exit(1)
    else:
        print("Échec du téléchargement de l'installeur.")
        input("Appuyez sur Entrée pour quitter...") # Pause
        sys.exit(1)
